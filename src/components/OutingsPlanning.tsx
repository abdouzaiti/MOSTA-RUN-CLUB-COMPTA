...
interface OutingsPlanningProps {
  ...
  onUpdateParticipant: (
    runId: string,
    runnerId: string,
    updates: Partial<RunParticipant>,
  ) => void;
  runners: Runner[];
...
export default function OutingsPlanning({
  ...
  const [destinationWilaya, setDestinationWilaya] =
    useState<string>("Hors Wilaya");
  ...
  const [logisticsSortBy, setLogisticsSortBy] = useState<"name" | "room">(
    "name",
  );
  ...
  const filteredRuns = sortedRuns.filter((run) => {
    ...
    const matchesSearch =
      ...
      (run.destinationWilaya &&
        run.destinationWilaya.toLowerCase().includes(term));

    const matchesDifficulty =
      difficultyFilter === "Tous" || run.difficulty === difficultyFilter;
    ...
    return (
      matchesSearch && matchesDifficulty && matchesDistance && matchesWilaya
    );
  });
  ...
  return (
    <div className={`space-y-6 ${language === "ar" ? "font-arabic" : ""}`}>
      ...
      <div
        className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${language === "ar" ? "sm:flex-row-reverse" : ""}`}
      >
        <div className={language === "ar" ? "text-right" : "text-left"}>
          <h2
            className={`text-xl font-serif italic font-bold text-natural-olive flex items-center gap-2 ${language === "ar" ? "flex-row-reverse" : ""}`}
          >
            <Flame className="w-5 h-5 text-natural-accent animate-pulse" />
            {language === "ar"
              ? "تخطيط الخرجات يداً بيد"
              : language === "en"
                ? "Side by Side Outings Planning"
                : "Planning des Sorties Côte à Côte"}
          </h2>
          <p className="text-xs text-natural-sage font-medium">
            {language === "ar"
              ? "انضم إلى خرجة جماعية أو خطط لواحدة لتحفيز الآخرين!"
              : language === "en"
                ? "Join a group run or plan one to motivate others!"
      ...
      {showForm && (
        <form
          ...
          className="bg-white rounded-3xl p-6 border border-natural-border space-y-4 animate-fade-in shadow-sm"
        >
          <div>
            <h3
              className={`text-sm font-bold text-natural-olive font-serif italic uppercase tracking-wider mb-1 ${language === "ar" ? "text-right" : ""}`}
            >
              {language === "ar"
                ? "تفاصيل الخرجة الجديدة"
                : "Détails du nouveau Run"}
            </h3>
            <p
              className={`text-xs text-natural-sage ${language === "ar" ? "text-right" : ""}`}
            >
              {language === "ar"
                ? "نموذج المسؤول لتخطيط المسارات الرسمية."
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className={language === "ar" ? "text-right" : ""}>
                <label className="block text-[11px] font-bold text-natural-olive mb-1 uppercase tracking-wider">
                  {language === "ar"
                    ? "عنوان الخرجة *"
                    : "Titre de la sortie *"}
                </label>
                <input
                  ...
                  placeholder={
                    language === "ar"
                      ? "مثال: الخرجة الوطنية الكبرى MRC"
                      : "Ex. Grande Sortie Nationale MRC"
                  }
              ...
              <div className={language === "ar" ? "text-right" : ""}>
                ...
                <input
                  ...
                  placeholder={
                    language === "ar"
                      ? "مثال: ساحة salamandre أمام الميناء"
                      : "Ex. Rond-point de Salamandre face au port"
                  }
                  className={`w-full text-xs px-3 py-2.5 bg-natural-bone border border-natural-border rounded-xl focus:outline-none focus:ring-1 focus:ring-natural-olive focus:border-natural-olive text-natural-text placeholder-natural-sage/70 ${language === "ar" ? "text-right" : ""}`}
                />
              </div>
              ...
              <div className={language === "ar" ? "text-right" : ""}>
                <label className="block text-[11px] font-bold text-natural-olive mb-1 uppercase tracking-wider">
                  {language === "ar" ? "ولاية الوجهة" : "Wilaya de Destination"}
                </label>
                <input
                  type="text"
                  ...
                  placeholder={
                    language === "ar"
                      ? "مثال: وهران، تيبازة، الجزائر (اتركها فارغة للمحلية)"
                      : "Ex. Oran, Tipaza, Alger (Vide pour Locale)"
                  }
            ...
            <div className="p-4 bg-emerald-50/40 rounded-2xl border border-emerald-200/60 space-y-3">
              <span className="text-xs font-bold text-emerald-800 uppercase tracking-wider flex items-center gap-1.5">
                ...
                🏷️ Grille Tarifaire (Transport & Lmbata / Nuitée par taille de
                chambre)
              </span>
            ...
            <div className="border border-natural-border rounded-2xl overflow-hidden bg-natural-bone/20">
              ...
              >
                <span
                  className={`flex items-center gap-1.5 font-mono ${language === "ar" ? "flex-row-reverse" : ""}`}
                >
                  {language === "ar"
                ...
                <div className="p-4 border-t border-natural-border bg-white space-y-4 animate-fade-in">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className={language === "ar" ? "text-right" : ""}>
                      <label className="block text-[10px] font-bold text-natural-olive mb-1 uppercase">
                        {language === "ar" ? "المسافة (كلم)" : "Distance (km)"}
                      </label>
                      <input
                        type="number"
                        min="1"
                    ...
                    <div className={language === "ar" ? "text-right" : ""}>
                      <label className="block text-[10px] font-bold text-natural-olive mb-1 uppercase">
                        {language === "ar" ? "الصعوبة" : "Difficulté"}
                      </label>
                      <select
                        value={difficulty}
                  ...
                  <div className={language === "ar" ? "text-right" : ""}>
                    ...
                    <textarea
                      ...
                      placeholder={
                        language === "ar"
                          ? "صف المسار، نقاط الظل، محطات التموين الممكنة..."
                          : "Décrivez l'itinéraire, les points d'ombre, les éventuelles pauses ravitaillement..."
                      }
          ...
          <div
            className={`flex justify-end gap-2 pt-2 ${language === "ar" ? "flex-row-reverse" : ""}`}
          >
            <button
              type="button"
              onClick={() => {
      ...
      <div className="bg-white rounded-3xl p-5 border border-natural-border shadow-sm mb-4">
        ...
        <div className="relative">
          <Search
            className={`absolute ${language === "ar" ? "right-3" : "left-3"} top-3.5 w-5 h-5 text-natural-olive`}
          />
          <input
            type="text"
      ...
      {filteredRuns.length === 0 ? (
        <div
          className={`p-12 text-center bg-white rounded-3xl border border-natural-border shadow-xs ${language === "ar" ? "font-arabic" : ""}`}
        >
          ...
          <p className="text-sm font-bold text-natural-olive font-serif italic text-base">
            {language === "ar"
              ...
              : language === "en"
                ...
                : "Aucun run ne correspond à vos filtres"}
          </p>
          <p className="text-xs text-natural-sage mt-1 font-medium">
            {language === "ar"
              ? "حاول تغيير كلمات البحث أو إعادة ضبط الفلاتر."
              : language === "en"
        ...
        <div className="grid grid-cols-1 gap-4">
          {filteredRuns.map((run) => {
            const isUserRegistered = run.participants.some(
              (p) => p.id === currentUser.id,
            );
            const isFull = run.maxParticipants
            ...
            return (
              <div
                ...
                className={`bg-white rounded-3xl border transition-all duration-300 ${
                  isExpanded
                    ? "border-natural-olive/40 shadow-sm ring-1 ring-natural-olive/20"
                    : "border-natural-border hover:border-natural-olive/30 shadow-xs"
                }`}
              >
                {/* Main run visual row */}
                <div className="p-5 md:p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex-1 space-y-2">
                    {/* Tags row */}
                    <div
                      className={`flex flex-wrap items-center gap-2 ${language === "ar" ? "flex-row-reverse" : ""}`}
                    >
                      <span
                        className={`text-xs font-bold px-3 py-1 rounded border ${getDifficultyStyles(run.difficulty)}`}
                      >
                  ...
                  <div className="flex items-center md:justify-end gap-3 md:gap-5 pt-3 md:pt-0 border-t md:border-t-0 border-natural-divider">
                    ...
                    >
                      ...
                      {run.elevationGain && run.elevationGain > 0 ? (
                        <span className="text-xs text-natural-sage font-bold block mt-0.5">
                          +{run.elevationGain}m d+
                        </span>
                      ) : null}
                    </div>
                    ...
                    <div className="px-3.5 py-2 bg-natural-bone rounded-xl text-center border border-natural-border">
                      <span className="text-xs text-natural-sage block font-bold">
                        {t("participants")}
                      </span>
                      <span className="text-sm font-bold text-natural-olive flex items-center gap-1 justify-center">
                        ...
                        {run.maxParticipants && (
                          <span className="text-natural-sage font-normal">
                            /{run.maxParticipants}
                          </span>
                        )}
                      </span>
                ...
                <div
                  ...
                  className={`px-5 py-3.5 bg-natural-sage-light/10 border-t border-natural-divider rounded-b-3xl flex items-center justify-between text-sm text-natural-sage transition select-none font-bold ${!run.completed ? "hover:bg-natural-sage-light/30 cursor-pointer" : ""}`}
                >
                  <p
                ...
                {isExpanded && !run.completed && (
                  <div className="p-5 md:p-6 bg-natural-bone/40 border-t border-natural-divider space-y-5 animate-fade-in text-xs rounded-b-3xl">
                    ...
                    {(currentUser.runClubRole === "Admin" ||
                      currentUser.runClubRole === "Coach") && (
                      <div
                        className={`bg-white border-2 border-natural-olive/35 rounded-2xl p-4.5 space-y-4 shadow-sm ${language === "ar" ? "text-right" : ""}`}
                      >
                        <div
                          className={`flex items-center justify-between border-b border-natural-divider pb-2.5 ${language === "ar" ? "flex-row-reverse" : ""}`}
                        >
                          <div
                            className={`flex items-center gap-1.5 ${language === "ar" ? "flex-row-reverse" : ""}`}
                          >
                            ...
                            <div>
                              <h4 className="font-bold text-natural-olive uppercase tracking-wide text-xs">
                                {language === "ar"
                                  ? "👑 لوحة الخدمات اللوجستية والميزانيات (خاصة بالمسؤولين والمدربين)"
                              ...
                              <p className="text-[10px] text-natural-sage font-semibold uppercase tracking-wider">
                                {run.isOrWilaya
                                  ? language === "ar"
                                    ? `خرجة وطنية لوجستية • ${run.destinationWilaya || "خارج الولاية"}`
                                    : `Sortie Nationale logistique • ${run.destinationWilaya || "Hors Wilaya"}`
                                  : language === "ar"
                                    ? "خرجة محلية • مستغانم"
                                    : "Sortie locale • Mostaganem"}
                              </p>
                            </div>
                          </div>
                          <div className="flex flex-col sm:flex-row items-end sm:items-center gap-2">
                            ...
                            {!run.completed && onCompleteRun && (
                              <button
                                ...
                                title={
                                  language === "ar"
                                    ? "إنهاء الخرجة (إغلاق التسجيلات)"
                                    : "Clôturer la sortie (fermer les inscriptions)"
                                }
                              >
                            ...
                            {onDeleteRun && (
                              <button
                                ...
                                title={
                                  language === "ar"
                                    ? "حذف الخرجة نهائياً"
                                    : "Supprimer la sortie définitivement"
                                }
                              >
                        ...
                        <div
                          className={`flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-natural-bone p-3 rounded-xl border border-natural-border ${language === "ar" ? "flex-row-reverse" : ""}`}
                        >
                          <span
                            className={`text-[11px] font-bold text-natural-olive flex items-center gap-1.5 shrink-0 ${language === "ar" ? "flex-row-reverse" : ""}`}
                          >
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              const matchedRunner = runners.find(
                                (r) => r.id === selectedId,
                              );
                              if (matchedRunner) {
                          ...
                          >
                            <option value="">
                              {language === "ar"
                                ? "-- اختر عداءً لإضافته --"
                                : "-- Sélectionner un athlète à ajouter --"}
                            </option>
                            {runners
                              .filter(
                                (r) =>
                                  !run.participants.some((p) => p.id === r.id),
                              )
                              .map((r) => (
                        ...
                        {run.participants.length === 0 ? (
                          <div className="p-4 text-center text-[11px] text-natural-sage italic border border-dashed border-natural-border rounded-xl">
                            Aucun participant inscrit pour l'instant. Choisissez
                            un athlète ci-dessus pour l'ajouter manuellement !
                          </div>
                        ) : (
                          <>
                            {/* Search bar for participants */}
                            <div
                              className={`mb-3 flex flex-col sm:flex-row gap-2 ${language === "ar" ? "flex-row-reverse" : ""}`}
                            >
                              <div className="relative group flex-1">
                                <Search
                                  className={`absolute ${language === "ar" ? "right-2.5" : "left-2.5"} top-2.5 w-3.5 h-3.5 text-natural-sage group-focus-within:text-natural-olive transition-colors`}
                                />
                                <input
                                  type="text"
                                  placeholder={
                                    language === "ar"
                                      ? "البحث عن عداء في هذه القائمة..."
                                      : "Rechercher un athlète dans cette liste..."
                                  }
                            ...
                            <div className="hidden md:block overflow-x-auto">
                              <table
                                className={`w-full text-left text-[11px] border-collapse min-w-[650px] ${language === "ar" ? "text-right" : ""}`}
                              >
                                <thead>
                                  <tr
                                    className={`border-b border-natural-border text-natural-sage font-bold font-mono tracking-wider ${language === "ar" ? "flex-row-reverse" : ""}`}
                                  >
                                    <th
                                      className={`py-2 ${language === "ar" ? "text-right" : "text-left"}`}
                                    >
                                      {language === "ar" ? "العداء" : "Athlète"}
                                    </th>
                                    <th
                                      className={`py-2 ${language === "ar" ? "text-right" : "text-left"}`}
                                    >
                                    ...
                                    <th
                                      className={`py-2 ${language === "ar" ? "text-left" : "text-right"}`}
                                    >
                                      {language === "ar"
                                        ? "إجمالي السعر"
                                        : "Prix Total"}
                                ...
                                <tbody className="divide-y divide-natural-divider">
                                  {[...run.participants]
                                    ...
                                    .map((partic) => {
                                      ...
                                      const costTransport = run.isOrWilaya
                                        ? run.transportPrice || 0
                                        : 0;

                                      const rType =
                                        partic.accommodationType || "room1";
                                      const costLodging = !hasLodging
                                        ? 0
                                        : rType === "room1"
                                      ...
                                      return (
                                        <tr
                                          key={partic.id}
                                          className="hover:bg-natural-bone/50 transition-colors"
                                        >
                                          ...
                                          <td className="py-2 text-right">
                                            <div className="flex flex-col items-end gap-0.5">
                                              <div className="flex items-center gap-1">
                                                ...
                                                />
                                                <span className="text-[9px] font-mono text-natural-sage">
                                                  DA
                                                </span>
                                              </div>
                                              {partic.customPrice !==
                              ...
                              {[...run.participants]
                                ...
                                .map((partic) => {
                                  ...
                                  return (
                                    <div
                                      key={partic.id}
                                      className="p-3 bg-natural-bone/20 border border-natural-border rounded-xl space-y-2.5 relative shadow-xs"
                                    >
                                      <div className="flex justify-between items-start">
                                        <div>
                                          <span className="font-bold text-[12px] text-natural-text block">
                                            {partic.name}
                                          </span>
                                      ...
                                      {hasLodging && (
                                        <div className="bg-emerald-50/55 p-2 rounded-lg border border-emerald-100 space-y-2">
                                          <div>
                                            <label className="block text-[9px] font-bold text-emerald-800 uppercase font-mono mb-1">
                                              Option de Chambre :
                                            </label>
                                            <select
                                              value={
                                      ...
                                      <div className="flex justify-between items-center bg-natural-bone/45 p-2.5 rounded-lg border border-natural-border/60">
                                        <span className="text-[10px] uppercase font-mono font-bold text-natural-olive">
                                          Tarif Dû :
                                        </span>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-natural-bone/40 p-3.5 rounded-xl border border-natural-border text-center">
                          <div className="bg-white p-2.5 rounded-lg border border-natural-border shadow-xxs">
                            <span className="text-[9px] text-natural-sage block font-mono font-bold uppercase leading-normal">
                              Total Athlètes
                            </span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div className="md:col-span-2 space-y-3">
                        <div>
                          <h4
                            className={`font-bold text-natural-olive uppercase tracking-widest font-mono text-[10px] mb-1 flex items-center gap-1.5 ${language === "ar" ? "flex-row-reverse text-right" : ""}`}
                          >
                          ...
                          <div className="p-4 bg-amber-50/40 rounded-2xl border border-amber-200/80 space-y-3">
                            <div className="flex items-center gap-2">
                              ...
                              <h4 className="text-xs font-black uppercase tracking-wider text-amber-800">
                                Logistique Sortie Nationale:{" "}
                                {run.destinationWilaya || "Hors Wilaya"}
                              </h4>
                            </div>
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-natural-olive">
                              <div className="bg-white/60 p-3 rounded-xl border border-amber-200 text-center">
                                <span className="font-mono text-[9px] block font-bold text-amber-900 uppercase">
                                  {language === "ar"
                                    ? "🚌 نقل النادي"
                                    : "🚌 Transport Club"}
                                </span>
                                <span className="font-serif italic font-extrabold text-sm">
                                  {run.transportPrice || 0} DA
                                </span>
                              </div>
                              <div className="bg-white/60 p-3 rounded-xl border border-amber-200 text-center">
                                <span className="font-mono text-[9px] block font-bold text-amber-900 uppercase">
                        ...
                        {isUserRegistered &&
                          ...
                          (() => {
                            ...
                            const transportChoice =
                              myParticipantEntry?.useTransport ?? true; // defaults to true
                            const accommodationChoice =
                              myParticipantEntry?.useAccommodation ?? false; // defaults to false
                            const currentBib =
                              myParticipantEntry?.bibNumber ?? "";
                          ...
                          <div className="p-4 bg-emerald-500/5 border border-emerald-500/15 rounded-xl space-y-2.5">
                            ...
                            <div className="grid grid-cols-3 gap-2 text-center text-[11px]">
                              <div className="bg-white p-2 rounded-lg border border-natural-border/60 shadow-xxs">
                                <span className="text-[9px] text-natural-sage font-mono block font-bold leading-normal">
                                  AUTOCAR CLUB
                                </span>
                              ...
                              <div className="bg-white p-2 rounded-lg border border-natural-border/60 shadow-xxs">
                                ...
                                <span className="font-black text-natural-olive font-mono text-xs">
                                  {
                                    run.participants.filter((p) => p.bibNumber)
                                      .length
                                  }{" "}
                                  / {run.participants.length}
                                </span>
                        ...
                        <div
                          className={`p-4 bg-natural-sage-light/35 border border-natural-border/70 rounded-xl space-y-1 ${language === "ar" ? "text-right" : ""}`}
                        >
                          <p
                            className={`font-bold text-natural-olive text-[11px] flex items-center gap-1 ${language === "ar" ? "flex-row-reverse" : ""}`}
                          >
                            {language === "ar"
                              ? "📢 تعليمات خاصة:"
                              : "📢 Consignes Spéciales :"}
                          ...
                          <ul
                            className={`list-disc space-y-1 text-natural-text font-medium text-[11px] ${language === "ar" ? "pr-4 list-inside" : "pl-4"}`}
                          >
                            <li>
                              {language === "ar"
                                ? "كن في الموعد: يبدأ شرح التمارين قبل 10 دقائق من الانطلاق."
                                : "Soyez à l'heure : le briefing des étirements commence 10min avant le départ."}
                            </li>
                            <li>
                              {language === "ar"
                                ? "أحضر زجاجة الماء الخاصة بك أو نظام ترطيب بسعة 500 مل على الأقل."
                                : "Emmenez votre bouteille d'eau ou système d'hydratation de 500ml minimum."}
                            </li>
                            <li>
                              {language === "ar"
                                ? "في حالة التأخر، اتصل بأحد المسؤولين أو أبلغنا على قناة النادي."
                                : "En cas de retard, contactez l'un des admins ou signalez-vous sur le canal du club."}
                            </li>
                          </ul>
                        </div>
                      </div>
                              ...
                              return (
                                <div
                                  ...
                                  className={`flex flex-col gap-2 p-2.5 rounded-xl border text-xs bg-white ${
                                    isMe
                                      ? "border-natural-olive/35 bg-natural-sage-light/20 shadow-xxs"
                                      : "border-natural-border/60"
                                  }`}
                                >
                                  {/* Info top row */}
                                  <div className="flex items-center justify-between gap-2">
                                    <div className="flex items-center gap-2 truncate">
                                      ...
                                      <div className="truncate">
                                        <p className="font-bold text-natural-text truncate text-[11px] flex items-center gap-1">
                                          {partic.name}
                                          {isMe && (
                                            <span className="text-[9px] text-natural-accent font-bold font-mono">
                                              (Vous)
                                            </span>
                                          )}
                                        </p>
                                        {partic.runClubRole &&
                                          partic.runClubRole !== "Membre" && (
                                            <span className="text-[9px] text-natural-accent font-bold uppercase tracking-wider block leading-none">
                                              {partic.runClubRole}
                                            </span>
                                          )}
                                      </div>
                                    ...
                                    <div className="flex items-center gap-1 shrink-0">
                                      {partic.bloodType && (
                                        <span
                                          className="text-[9px] font-mono font-black px-1.5 py-0.2 bg-rose-50 text-rose-600 rounded border border-rose-100 shrink-0"
                                    ...
                                    {(() => {
                                      ...
                                      return (
                                        <div className="flex items-center gap-1 group">
                                          <span
                                            className={`text-[9px] font-mono font-black px-1.5 py-[1px] rounded leading-none shrink-0 ${
                                              partic.bibNumber
